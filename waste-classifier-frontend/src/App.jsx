...
function App() {
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      const res = await axios.post('https://waste-classifier-fyp.onrender.com/api/predict', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });

      if (res.data.confidence < 0.4) {
        setError('Low confidence (below 40%). Please try again with a clearer image.');
        setPrediction(null);
      } else {
